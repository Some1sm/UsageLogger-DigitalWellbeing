...
namespace UsageLogger.Helpers
{
    ...
    public class FocusManager
    {
            ...
            try
            {
                ...
                switch (activeSession.Mode)
                {
                    case FocusMode.Chill:
                        ...
                        ShowChillNotification(activeSession, "UsageLogger (Startup)");
                        break;
        ...
        private async Task ScanAndCacheAppsAsync(int days)
        {
            try
            {
                string logsFolder = UsageLogger.Core.ApplicationPath.UsageLogsFolder;
                var repo = new UsageLogger.Core.Data.AppSessionRepository(logsFolder);
